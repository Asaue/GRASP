...
class PaDTDecoder(nn.Module):
    def __init__(self, config, dtype):
        ...
        # [新增] Point Regression Head
        # 输入是 High Res Patch Feature，输出是 (x, y) 局部坐标
        self.point_prediction = nn.Sequential(
            nn.Linear(config['hidden_size'], config['hidden_size']),
            nn.GELU(),
            nn.Linear(config['hidden_size'], 2), # 输出 x, y
    ...
    def forward(self, object_vp_feat, cu_low_res_feats, cu_high_res_feats, visual_pe, cu_patch, obj_image_grid_thws, device):
        ...
        # [新增] 预测每个Patch内部的关键点坐标 (x, y)
        # cu_high_res_feats: [Total_Patches, Hidden_Size] -> pred_points_local: [Total_Patches, 2]
        pred_points_local = self.point_prediction(cu_high_res_feats)
        ...
        if self.use_mask_loss is False:
            # 注意：这里也需要返回 pred_points_local，或者返回 None
            return bbox_output, score_output, None, (), pred_points_local
        ...
        # [修改] 返回值增加了 pred_points_local
        return bbox_output, score_output, masks_per_object, (Hs, Ws), pred_points_local
