...
# 模型权重路径
export CHECKPOINT='/Data/Docker_liuwu/models/checkpoints/PaDT-REC-3B_crack'

# 日志后缀 (输出文件会以此结尾，例如 ..._padt_crack.json)
export LOG_SUFFIX='padt_crack'
...
do
    ...
    CUDA_VISIBLE_DEVICES=0,1,2,3 torchrun \
        --master_port="12375" \
        --nproc_per_node=4 \
        inference_refcrack.py "$CHECKPOINT" "$SPLIT" "$LOG_SUFFIX"
    ...
    python eval_refcrack.py "$LOG_SUFFIX" "$SPLIT"
